            ...
            html.A(
                ...
                dbc.Row(
                    [
                        ...
                        dbc.Col(dbc.NavbarBrand("Logo", className="ms-2", id="logo_country")),
                    ],
                    align="center",
                    className="g-0",
                ),
...
def update_output(value):
    ...
    df_gbr = df_gbr[(df_gbr.game_season == 'Summer') & (df_gbr.country_3_letter_code == code_country)]
    
    medal_per_date = df_gbr.groupby(['game_year'])['medal_type'].count().reset_index()
...
def update_output(value):
    df_gbr = df_result.merge(df_host, left_on='slug_game', right_on='game_slug')
    df_gbr = df_gbr[(df_gbr.game_season == 'Summer') & (df_gbr.country_3_letter_code == value)]
